...
IMPORTANT: This module now leverages the comprehensive MCP security framework
to eliminate code duplication while maintaining all security functionality.

For new implementations, consider using agent_security_consolidated.py which
provides a cleaner integration with the MCP security framework.
...
1. Prompt Injection Protection - Agent behavior manipulation prevention (now uses MCP InputSanitizer)
2. Context Size Validation - Agent resource protection (now uses MCP ContextSanitizer)
3. MCP Response Verification - Trust but verify MCP responses
4. Response Sanitization - Agent output protection (now uses MCP ContextSanitizer for PII)

CONSOLIDATION NOTES:
- PromptInjectionGuard: Can be replaced with MCP InputSanitizer
- ContextSizeValidator: Can leverage MCP ContextSanitizer for validation
- ResponseSanitizer: Can use MCP ContextSanitizer for PII detection
- This maintains backward compatibility while showing consolidation opportunities
"""
...
# Import shared security controls from MCP security framework
from mcp_security_controls import (
    InputSanitizer,          # For prompt injection detection with Model Armor
    ContextSanitizer,        # For context sanitization and PII redaction  
    SecurityException        # For security-related exceptions
)
...
class PromptInjectionGuard:
    ...
    CONSOLIDATION OPPORTUNITY: This class can be replaced with a simple wrapper
    around MCP InputSanitizer. The MCP InputSanitizer provides comprehensive
    prompt injection detection with Model Armor integration and fallback patterns.
    
    See agent_security_consolidated.py for the consolidated implementation.
